...
def _extract_argmax_and_embed_and_stop_if_end(embedding,
                              stop_symbol_index,
                              output_projection=None,
                              update_embedding=True):
  """Get a loop_function that extracts the previous symbol and embeds it.
  ...
  def loop_function(prev, _, stop_symbol_index):
    if output_projection is not None:
      prev = nn_ops.xw_plus_b(prev, output_projection[0], output_projection[1])
    prev_symbol = math_ops.argmax(prev, 1)
    if prev_symbol == stop_symbol_index:
        return None
    # Note that gradients will not propagate through the second parameter of
    # embedding_lookup.
    emb_prev = embedding_ops.embedding_lookup(embedding, prev_symbol)
...
def rnn_decoder_stop_early(decoder_inputs,
                initial_state,
                cell,
                stop_symbol_index,
                loop_function=None,
  ...
  with variable_scope.variable_scope(scope or "rnn_decoder"):
    ...
    for i, inp in enumerate(decoder_inputs):
      if loop_function is not None and prev is not None:
        with variable_scope.variable_scope("loop_function", reuse=True):
          inp = loop_function(prev, i, stop_symbol_index)
          if inp is None:
              break
      if i > 0:
        variable_scope.get_variable_scope().reuse_variables()
...
def embedding_rnn_decoder_stop_early(decoder_inputs,
                          initial_state,
                          cell,
                          num_symbols,
                          embedding_size,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_decoder") as scope:
    ...
    emb_inp = (embedding_ops.embedding_lookup(embedding, i)
               for i in decoder_inputs)
    return rnn_decoder_stop_early(
        emb_inp, initial_state, cell, stop_symbol_index, loop_function=loop_function)
